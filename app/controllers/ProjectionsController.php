...
class ProjectionsController extends Controller
{
    ...
    public function details($projection_id,$contractor_id)
    {
        ...
        if ($projection_id == 43 or $projection_id == 44 or $projection_id == 45 or $projection_id == 46) {
            ...
            if ($projection != null and (in_array($projection_id, [1, 23,2, 27,3,42,19,20,21,22,24,25,26,28,29,30]))){
                $this->f3->set('proj_root_id', $projection[0]['prj_root_id']);
                $path = 'r_json/proj_' . $projection_id . '.json';
                $file_content = json_decode($this->f3->read($path));
                $caption = $file_content->chart->caption;
    ...
    public function ShowCategory($PARAMS){
        ...
        $id_pro = $this->f3->get('PARAMS.id_pro');

        if((in_array($id_pro, [19,20,21,22,4,5,44,46,15,16,17,37,38]))){
            $this->f3->set('id_tap',1);
        }elseif((in_array($id_pro, [23,24,25,26,8,9]))){
            $this->f3->set('id_tap',2);
        }elseif((in_array($id_pro, [27,28,29,30,6,7]))){
            $this->f3->set('id_tap',3);
        }elseif((in_array($id_pro, [10,11]))){
            $this->f3->set('id_tap',4);
        }elseif((in_array($id_pro, [12,13]))){
            $this->f3->set('id_tap',5);
        }
