...
# === path to json file 
path <- "RCODE/ind_23.json"

# === read chart json file 
chart <- read_json("ind_23_chart.json")
...
#========================== ind1-1 =================================
data <- data_all %>% 
  filter((P_ENDD+extra_time) >= input1[1] & (P_ENDD+extra_time) <= input1[2] & PROGRESS_A <= 100 ) %>%
  mutate(progress_indx = cut(PROGRESS_A,breaks = c(0,1,25,50,75,100,101), labels = 1:6, right = FALSE) ) %>%
  group_by(label = progress_indx) %>%
  summarize(number_contr = length(unique(cont_name) ),
            number_projects = n())
...
data$label <- unlist(chart$colabel)
