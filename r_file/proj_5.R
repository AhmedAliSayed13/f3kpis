...
# === path to json file 
path <- "RCODE/proj_5.json"

# === read table head
chart <- read_json("proj_5_chart.json")
...
#================ proj_5 ======================
#### التنبؤ بقيمة المشروع (بالمليون ريال) كقيمة متوقعة مع فترة ثقة 95% علي مستوي كل ادارة تعليمية
data <- data_all  %>% group_by(area_name) %>% 
  summarise(expected = quantile(P_VALUE,  probs = c(0.50), na.rm = T) /1000000,
            lower_interval = quantile(P_VALUE,  probs = c(0.05), na.rm = T) /1000000,
            uper_interval = quantile(P_VALUE,  probs = c(0.95), na.rm = T) /1000000) %>% 
