...
# === path to json file 
path <- "RCODE/proj_14.json"

# === read table head
chart <- read_json("proj_14_chart.json")
...
model <- lm(log(P_DURATION) ~ factor(reg_name)+log(P_VALUE), data = lm1data) 

lm1data$duration_pred <- exp(fitted(model))

data = lm1data %>% select(reg_name, P_VALUE, duration_pred) %>% 
  mutate_if(is.numeric, round, 3) %>% head(n=50)
