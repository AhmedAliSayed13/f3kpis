...
# === path to json file 
path <- "RCODE/ind_40.json"

# === read chart json file 
chart <- read_json("ind_40_chart.json")
...
#========================== ind1-1 =================================
# requires the begining of current year (meeladi) as hijri
if(year(today())==2020) yearcond = "1441-06-06"
if(year(today())==2021) yearcond = "1442-05-17"
# requires the begining of current year (meeladi) as hijri
data <- data_all %>% 
  group_by(label = reg_name) %>%
  summarise(number_projects = sum(CH_DATE[CH_CASE==1] > yearcond, na.rm = T) )
