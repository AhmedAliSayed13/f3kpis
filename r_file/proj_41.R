...
# === path to json file 
path <- "RCODE/proj_41.json"

# === read table head
chart <- read_json("proj_41_chart.json")
...
data <- data_all %>% group_by(cont_name) %>%
  ...
  mutate_if(is.numeric, round, 2) %>%  filter(cont_name == input_name) %>%
  select(cont_name, prob_mashouba, prob_faskh, prob_1warning, prob_2warning, prob_3warning   )
