...
# === path to json file 
path <- "RCODE/ind_15.json"

# === read chart json file 
chart <- read_json("ind_15_chart.json")
...
#   group_by(reg_name) %>%
#   summarise(progress_100per = 100*sum(PROGRESS_A < 50, na.rm = T)/n() )%>%
...
data <- data_all %>% 
  ...
  summarise(progress_100 = sum(PROGRESS_A < 50, na.rm = T),
