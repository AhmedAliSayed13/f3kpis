...
# === path to files 
path1 <- "RCODE/report_27.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_27.html"
# tble_head <- read.table("report_27_head.txt", encoding = "UTF-8", sep = "\t")
tble_head <- read.table("report_27_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  summarise(number_warnings = sum(P_warn_index,na.rm = T ),
            number_1warn = sum(P_1warn_index, na.rm = T),
            number_2warn = sum(P_2warn_index, na.rm = T),
            number_3warn = sum(P_3warn_index, na.rm = T)) %>% 
  mutate_if(is.numeric, round,3) %>% arrange(desc(number_warnings))
