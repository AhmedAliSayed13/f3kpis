...
# === path to files 
path1 <- "RCODE/report_32.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_32.html"
tble_head <- read.table("report_32_head.txt", sep = "\t")
...
data = data_all  %>%
  ...
  group_by(area_name,project_status) %>%
  summarise(number_projects_area = unique(number_projectsR),
            number_projects = n() ) %>%
  spread(project_status,number_projects) %>% filter(!is.na(area_name) )
