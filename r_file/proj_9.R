...
# === path to json file 
path <- "RCODE/proj_9.json"

# === read table head
chart <- read_json("proj_9_chart.json")
...
#================ proj_9 ======================
#### التنبؤ بقيمة المشروع الاضافية (بالألف ريال) كقيمة متوقعة مع فترة ثقة 95% علي مستوي كل ادارة تعليمية
data <- data_all %>%  group_by(area_name) %>% filter(extra_index > 0) %>% 
  summarise(expected = quantile(extra_value,  probs = c(0.50), na.rm = T)/1000 ,
            lower_interval = quantile(extra_value,  probs = c(0.05), na.rm = T)/1000 ,
            uper_interval = quantile(extra_value,  probs = c(0.95), na.rm = T)/1000 ) %>% 
