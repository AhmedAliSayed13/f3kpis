...
# === path to json file 
path <- "RCODE/ind_10.json"

# === read chart json file 
chart <- read_json("ind_10_chart.json")
...
#   group_by(reg_name) %>%
#   summarise(number_projects_loc = sum(P_loc_index, na.rm = T) )
# 
...
data <- data_all %>% 
  ...
  summarise(number_projects_loc = sum(P_loc_index, na.rm = T),
            perc_projects_loc = 100*number_projects_loc/n())  %>%
