...
# === path to json file 
path <- "RCODE/ind_7.json"

# === read chart json file 
chart <- read_json("ind_7_chart.json")
...
#========================== ind7-1 =================================
data = data_all %>% 
  filter((P_ENDD+extra_time) >= input1[1] & (P_ENDD+extra_time) <= input1[2] & !is.na(project_status)) %>%
  group_by(label = reg_name, project_status) %>%
  summarise(number_projects = n() )

data <- data %>% spread(project_status, number_projects)
data[is.na(data)] = 0
data[] <- lapply(data, as.character )

data2 <- data[2]; names(data2) <- "value"
data3 <- data[3]; names(data3) <- "value"
data4 <- data[4]; names(data4) <- "value"
data5 <- data[5]; names(data5) <- "value"

categories <- list(list(category = data[1])) # label
dataset <- list( list(seriesname = colnames(data)[2], data = data2),
                 list(seriesname = colnames(data)[3], data = data3),
                 list(seriesname = colnames(data)[4], data = data4),
                 list(seriesname = colnames(data)[5], data = data5) ) # data
