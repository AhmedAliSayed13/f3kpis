...
# === path to json file 
path <- "RCODE/proj_8.json"

# === read table head
chart <- read_json("proj_8_chart.json")
...
#================ proj_8 ======================
#### التنبؤ بقيمة المشروع الاضافية (بالألف ريال) كقيمة متوقعة مع فترة ثقة 95% علي مستوي كل منطقة
data <- data_all %>%  group_by(reg_name) %>% filter(extra_index > 0) %>% 
