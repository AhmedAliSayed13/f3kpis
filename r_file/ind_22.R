...
# === path to json file 
path <- "RCODE/ind_22.json"

# === read chart json file 
chart <- read_json("ind_22_chart.json")
...
data <- data_all %>% 
  ...
  summarise(number_projects = n(),
            perc_projects = 100*number_projects/total.number) %>%
  mutate_if(is.numeric, round,3) %>% arrange(desc(number_projects)) %>% head(n=30)
