...
# === path to files 
path1 <- "RCODE/report_28.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_28.html"
tble_head <- read.table("report_28_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  summarise(n_war = sum(P_1warn_index, na.rm = T)+sum(P_2warn_index, na.rm = T)+sum(P_3warn_index, na.rm = T),
            n_war_1 = sum(P_1warn_index, na.rm = T),
            n_war_2 = sum(P_2warn_index, na.rm = T),
            n_war_f = sum(P_3warn_index, na.rm = T)) %>%
  mutate_if(is.numeric, round, 3) %>% filter(P_CONTID == input1[3]) %>%
  select(label, n_war, n_war_1, n_war_2, n_war_f )
