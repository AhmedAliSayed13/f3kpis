...
# === path to files 
path1 <- "RCODE/report_3.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_3.html"
tble_head <- read.table("report_3_head.txt", sep = "\t")
...
data = data_all %>% 
  ...
  group_by(reg_name,project_status) %>%
  summarise(number_projects_reg = unique(number_projectsR),
            number_projects = n() ) %>%
  spread(project_status,number_projects)
