...
# === path to files 
path1 <- "RCODE/report_24.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_24.html"
tble_head <- read.table("report_24_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  summarise(number_projects = n(),
            number_projects_prim = sum(P_prim_index, na.rm = T),
            number_projects_final = sum(P_final_index, na.rm = T),
            ...
            number_3warn = sum(P_3warn_index, na.rm = T)) %>% 
  mutate_if(is.numeric, round,3) %>% arrange(desc(number_projects))
