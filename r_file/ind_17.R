...
# === path to json file 
path <- "RCODE/ind_17.json"

# === read chart json file 
chart <- read_json("ind_17_chart.json")
...
data <- data_all %>% 
  ...
  summarise(sum_extra_duration = sum(extra_time, na.rm = T),
            perc_extra_duration = 100*sum_extra_duration/sum(P_DURATION, na.rm = T)) %>%
