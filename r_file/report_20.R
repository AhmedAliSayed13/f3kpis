...
# === path to files 
path1 <- "RCODE/report_20.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_20.html"
tble_head <- read.table("report_20_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  group_by(reg_name) %>% filter(is.na(P_PRIMAD) ) %>%
  summarise(number_projectsg = n(),
