...
# === path to json file 
path <- "RCODE/proj_7.json"

# === read table head
chart <- read_json("proj_7_chart.json")
...
#================ proj_7 ======================
#### التنبؤ بمدة المشروع كقيمة متوقعة بفترة ثقة 95% علي مستوي كل ادارة تعليمية
data <- data_all %>% group_by(area_name) %>% 
  summarise(expected = quantile(P_DURATION,  probs = c(0.50), na.rm = T) ,
            lower_interval = quantile(P_DURATION,  probs = c(0.05), na.rm = T) ,
            uper_interval = quantile(P_DURATION,  probs = c(0.95), na.rm = T) ) %>% 
