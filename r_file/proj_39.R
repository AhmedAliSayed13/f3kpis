...
# === path to json file 
path <- "RCODE/proj_39.json"

# === read table head
chart <- read_json("proj_39_chart.json")
...
#================ proj_19 ======================
## تقدير مستوي أداء المقاولين
# requires input: contractor ID
good.index = function(x){ y = (x-min(x,na.rm = T))/(max(x,na.rm = T)-min(x,na.rm = T)); y}
bad.index = function(x){ y = (max(x,na.rm = T)-x)/(max(x,na.rm = T)-min(x,na.rm = T)); y}

data <- data_all %>% group_by(cont_name) %>%
  summarise(n_p = n(),
            n_p_P = sum(P_prim_index, na.rm = T),
            n_p_f = sum(P_final_index, na.rm = T) ,
            n_p_mas = sum(P_mashouba_index, na.rm = T),
            n_p_fas = sum(P_faskh_index, na.rm = T), 
            mean_progress = mean(PROGRESS_A, na.rm = T),
            n_war = sum(P_1warn_index, na.rm = T)+sum(P_2warn_index, na.rm = T)+sum(P_3warn_index, na.rm = T),
            n_war_f = sum(P_3warn_index, na.rm = T)) %>%
  mutate(indx = 100*(good.index(n_p)+good.index(n_p_P)+good.index(n_p_f)+
                       bad.index(n_p_mas)+bad.index(n_p_fas)+
                       good.index(mean_progress)+bad.index(n_war)+bad.index(n_war_f))/8) %>%
  mutate_if(is.numeric, round, 3) %>% filter(cont_name == input_name, !is.na(indx)) 
...
# names(data) <- c("value", rep("value",ncol(data)-1 ) )
# 
# categories <- list(list(category = data[1])) # label
# dataset <- c()
# for (i in 1:ncol(data)){
#   dataset[[i]] = c(seriesname = colhead[i], list(data = data[i]) ) }
...
# unique(data_all$cont_name[data_all$P_CONTID==input])
