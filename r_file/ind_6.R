...
# === path to json file 
path <- "RCODE/ind_6.json"

# === read chart json file 
chart <- read_json("ind_6_chart.json")
...
data = data_all %>% 
  ...
  summarise(percent_projects = 100*n()/total.number ) %>%
...
dataset <- list( list(seriesname='status', data = data[2])   ) # data
