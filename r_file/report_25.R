...
# === path to files 
path1 <- "RCODE/report_25.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_25.html"
tble_head <- read.table("report_25_head.txt", sep = "\t")
...
# input1 <- c("1426-01-11", "1449-10-11", 41) # year


#================ report_25 ======================
data1 <- data_all %>% 
  filter((P_ENDD+extra_time) >= input1[1] & (P_ENDD+extra_time) <= input1[2] ) %>%
  group_by(label = reg_name) %>%
  summarise(sum_value = sum(P_VALUE, na.rm = T)/1e6  )#,
#number_cont = length(unique(cont_name)) )

data2 <- data_all %>% 
  filter((P_ENDD+extra_time) >= input1[1] & (P_ENDD+extra_time) <= input1[2] ) %>%
  group_by(label = reg_name, P_CONTID) %>%
  summarise(number_projects = n(),
            sum_value_c = sum(P_VALUE, na.rm = T)/1e6)# %>%
#slice_max(number_projects, n = 5) %>%
#summarise(number_cont_top = n(),
#          number_projects_top = sum(number_projects))

data <- data1 %>% inner_join(data2, by=("label")) %>%
  mutate(perc_value = 100*sum_value_c/sum_value  ) %>% #,
  #       perc_projects_top = 100*number_projects_top/number_projects) %>%
  mutate_if(is.numeric, round,3) %>% filter(P_CONTID == input1[3]) %>%
  select(label, number_projects, sum_value_c, perc_value )
