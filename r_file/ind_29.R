...
# === path to json file 
path <- "RCODE/ind_29.json"

# === read chart json file 
chart <- read_json("ind_29_chart.json")
...
data <- data_all %>% 
  ...
  group_by(cont_name) %>%
  summarise(sum_extra_value = sum(extra_value, na.rm = T),
            percent_extra_value = 100*sum_extra_value/sum(P_VALUE, na.rm = T) ) %>% 
  arrange(desc(sum_extra_value)) %>% filter(percent_extra_value <= 100) %>%
  mutate_if(is.numeric, round,3) %>% 
  select(label = cont_name, value = percent_extra_value) %>% head(n=30)

data[] <- lapply(data, as.character )

categories <- list(list(category = data[1])) # label
dataset <- list( list(seriesname='contr', data = data[2])   ) # data
