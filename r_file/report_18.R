...
# === path to files 
path1 <- "RCODE/report_18.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_18.html"
tble_head <- read.table("report_18_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  summarise(number_projectsg = n(),
            sum_extra_value = sum(extra_value, na.rm = T)/1e6,
            percent_extra_value = 100*sum(extra_value, na.rm = T)/sum(P_VALUE, na.rm = T)) %>% 
