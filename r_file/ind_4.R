...
# === path to json file 
path <- "RCODE/ind_4.json"

# === read chart json file 
chart <- read_json("ind_4_chart.json")
...
#========================== ind4-1 =================================
data <- data_all %>% 
  filter((P_ENDD+extra_time) >= input1[1] & (P_ENDD+extra_time) <= input1[2] & !is.na(project_gender) ) %>%
  group_by(label=reg_name, project_gender) %>%
  summarise(number_projects = n() ) 

data = data %>% spread(project_gender, number_projects)
data[is.na(data)] = 0
colmnames <- colnames(data)

names(data) <- c("label", "value1", "value2")
data <- data %>%
  summarize(value1_per = 100*value1/(value1+value2),
            value2_per = 100*value2/(value1+value2)) %>%
...
dataset <- list( list(seriesname = colmnames[2], data = data2),
                 list(seriesname = colmnames[3], data = data3)  ) # data
