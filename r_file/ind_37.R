...
# === path to json file 
path <- "RCODE/ind_37.json"

# === read chart json file 
chart <- read_json("ind_37_chart.json")
...
data <- data_all %>% 
  ...
  summarise(number_projects = sum( (as.Date("1442-05-15") - (P_ENDD+extra_time))/30 > 18 & 
                                     (as.Date("1442-05-15") - (P_ENDD+extra_time))/30 < 48, na.rm = T) ) 
