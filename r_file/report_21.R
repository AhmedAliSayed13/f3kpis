...
# === path to files 
path1 <- "RCODE/report_21.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_21.html"
tble_head <- read.table("report_21_head.txt", sep = "\t")
...
data <- data1 %>% inner_join(data2, by=("label")) %>%
  mutate(perc_projects = 100*number_projects.y/number_projects.x  ) %>% #,
  #       perc_projects_top = 100*number_projects_top/number_projects) %>%
  mutate_if(is.numeric, round,3) %>% filter(P_CONTID == input1[3]) %>%
  select(label, number_projects.x, number_projects.y, perc_projects )
