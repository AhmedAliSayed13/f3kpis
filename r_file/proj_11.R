...
# === path to json file 
path <- "RCODE/proj_11.json"

# === read table head
chart <- read_json("proj_11_chart.json")
...
#================ proj_11 ======================
#### التنبؤ بمدة المشروع الاضافية كقيمة متوقعة بفترة ثقة 95% علي مستوي كل ادارة تعليمية
data <- data_all %>%  group_by(area_name) %>% filter(extra_index > 0) %>% 
  summarise(expected = quantile(extra_time,  probs = c(0.50), na.rm = T) ,
            lower_interval = quantile(extra_time,  probs = c(0.05), na.rm = T) ,
            uper_interval = quantile(extra_time,  probs = c(0.95), na.rm = T) ) %>% 
