...
# === path to json file 
path <- "RCODE/ind_18.json"

# === read chart json file 
chart <- read_json("ind_18_chart.json")
...
data <- data_all %>% 
  ...
  summarise(number_projects = n(),
            number_projectsF =  100*sum( project_gender == unique(project_gender)[1] )/n(),
            number_projectsM =  100*sum( project_gender == unique(project_gender)[2] )/n()) %>% 
  mutate(percN = 100*number_projects/c(tapply(number_projects,reg_name,sum))) %>% 
  ungroup() %>%  mutate_if(is.numeric, round,3) %>% filter(!is.na(area_name))
