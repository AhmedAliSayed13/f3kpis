...
# === path to json file 
path <- "RCODE/ind_14.json"

# === read chart json file 
chart <- read_json("ind_14_chart.json")
...
#   group_by(reg_name) %>%
#   summarise(progress_100per = 100*sum(PROGRESS_A==100,na.rm = T)/n() )%>%
#   mutate_if(is.numeric, round,3)
# 
# names(data) <- c("label", "value")
# data[] <- lapply(data, as.character )
# 
# categories <- list(category = data[1]) # label
...
data <- data_all %>% 
  ...
  summarise(progress_100 = sum(PROGRESS_A==100,na.rm = T),
            progress_100per = 100*sum(PROGRESS_A==100,na.rm = T)/n() ) %>%
  mutate_if(is.numeric, round,3)
