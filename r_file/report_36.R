...
# === path to files 
path1 <- "RCODE/report_36.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_36.html"
tble_head <- read.table("report_36_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  summarise(number_projects_reg = unique(number_projectsR),
            number_projects = sum( (P_ENDD+extra_time) > "1442-05-15", na.rm = T),
            percent_projects = 100*number_projects/number_projects_reg) %>% 
