...
# === path to json file 
path <- "RCODE/proj_15.json"

# === read table head
chart <- read_json("proj_15_chart.json")
...
#================ proj_15 ======================
# حساب احتمالات الخصائص الحرجة للمشاريع
#من أهم الخصائص الحرجة للمشروع الحصول علي انذارات او السحب أو فسخ العقد، وبالتالي تقدير احتماليةة الخصائص يعتمد من المعلومات المهمة لمتخذ القرار.
#فيما يلي يمكن استخدام فكرة الاحتمالات في تقدير احتمالات هذه الخصائص الحرجة للمشاريع.


### حساب احتمالات الخصائص الحرجة للمشاريع علي مستوي المناطق
data <- data_all %>% group_by(reg_name) %>%
  summarise(number_projects = n(),
            number_warning = sum(P_warn_index, na.rm = T),
            prob_warning = 100*sum(P_warn_index > 0 ,na.rm=T)/number_projects,
            number_1warning = sum(P_1warn_index, na.rm = T),
            prob_1warning = 100*sum(P_1warn_index > 0 ,na.rm=T)/number_projects,
            number_2warning = sum(P_2warn_index, na.rm = T),
            prob_2warning = 100*sum(P_2warn_index > 0 ,na.rm=T)/number_projects,
            number_3warning = sum(P_3warn_index, na.rm = T),
            prob_3warning = 100*sum(P_3warn_index > 0 ,na.rm=T)/number_projects,
            number_projects_mashouba = sum(P_mashouba_index, na.rm = T),
            prob_mashouba = 100*number_projects_mashouba/number_projects,
            number_projects_faskh = sum(P_faskh_index, na.rm = T),
            prob_faskh = 100*number_projects_faskh/number_projects) %>%  
  mutate_if(is.numeric, round, 2) %>%
  select(reg_name, prob_mashouba, prob_faskh, prob_1warning, prob_2warning, prob_3warning   )
