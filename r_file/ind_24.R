...
# === path to json file 
path <- "RCODE/ind_24.json"

# === read chart json file 
chart <- read_json("ind_24_chart.json")
...
data <- data_all %>% 
  ...
  #select(cont_name, indx) %>% 
  arrange(desc(indx)) %>% head(n=30) #%>% head(input$maxrows1) #%>% datatable()

data[] <- lapply(data, as.character )

#colmnames <- names(data)
colhead <- chart$head
