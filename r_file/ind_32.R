...
# === path to json file 
path <- "RCODE/ind_32.json"

# === read chart json file 
chart <- read_json("ind_32_chart.json")
...
data <- data_all %>% 
  ...
  group_by(label = cont_name) %>% filter(is.na(P_FIND) ) %>%
  summarise(number_warnings = sum(P_warn_index,na.rm = T ),
            number_warningsFin = sum(P_3warn_index,na.rm = T ),
            number_projects = n() ) %>% arrange(desc(number_projects)) %>%
...
dataset <- list( list(seriesname = colhead[[1]], data = data2),
                 list(seriesname = colhead[[2]], data = data3),
                 list(seriesname = colhead[[3]], data = data4) ) # data
