...
# === path to json file 
path <- "RCODE/ind_27.json"

# === read chart json file 
chart <- read_json("ind_27_chart.json")
...
#========================== ind1-1 =================================
total.value  = as.numeric( data_all %>% summarise( sum(P_VALUE, na.rm = T) )  ) 
data <- data_all %>% 
  group_by(reg_name, area_name) %>%
  summarise(number_projects = n(),
            sum_value = sum(P_VALUE, na.rm = T),
            percent_value = 100*sum(P_VALUE, na.rm = T)/total.value) %>% 
  mutate(percN = 100*number_projects/c(tapply(number_projects,reg_name,sum))) %>% 
  mutate(percV = 100*sum_value/c(tapply(sum_value,reg_name,sum))) %>% ungroup() %>%
  select(label = area_name, percN, percV) %>%
  mutate_if(is.numeric, round,3) %>% filter(!is.na(area_name))

data[] <- lapply(data, as.character )
colhead <- chart$head

data2 <- data[2]; names(data2) <- "value"
data3 <- data[3]; names(data3) <- "value"

categories <- list(list(category = data[1])) # label
dataset <- list( list(seriesname = colhead[[1]], data = data2),
                 list(seriesname = colhead[[2]], data = data3)  ) # data
