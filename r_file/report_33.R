...
# === path to files 
path1 <- "RCODE/report_33.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_33.html"
tble_head <- read.table("report_33_head.txt", sep = "\t")
...
data = data_all %>% 
  ...
  filter(!is.na(CH_CASE) ) %>%
  group_by(area_name,CH_CASE) %>%
  summarise(number_projects_area = unique(number_projectsR),
            number_projects = n() ) %>%
  spread(CH_CASE,number_projects) %>% filter(!is.na(area_name) ) 
