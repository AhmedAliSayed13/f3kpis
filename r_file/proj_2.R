...
# === path to json file 
path <- "RCODE/proj_2.json"

# === read table head
chart <- read_json("proj_2_chart.json")
...
dataset <- c()
dataset[[1]] = c(seriesname = colhead[1], list(data = data1[2:3]) ) 
dataset[[2]] = c(seriesname = colhead[2], list(data = data2[2:3]) ) 
dataset[[3]] = c(seriesname = colhead[3], list(data = data3[2:3]) ) 
