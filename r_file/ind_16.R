...
# === path to json file 
path <- "RCODE/ind_16.json"

# === read chart json file 
chart <- read_json("ind_16_chart.json")
...
#========================== ind1-1 =================================
total.duration  = as.numeric( data_all %>% summarise( sum(P_DURATION, na.rm = T) )  ) 
data <- data_all %>% 
  filter((P_ENDD+extra_time) >= input1[1] & (P_ENDD+extra_time) <= input1[2] ) %>%
  group_by(label = reg_name) %>%
  summarise(sum_duration = sum(P_DURATION, na.rm = T),
            perc_duration = 100*sum(P_DURATION, na.rm = T)/total.duration) %>%
