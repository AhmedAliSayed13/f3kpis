...
# === path to json file 
path <- "RCODE/ind_9.json"

# === read chart json file 
chart <- read_json("ind_9_chart.json")
...
#   group_by(reg_name) %>%
#   summarise(number_projects_prim = sum(P_prim_index, na.rm = T) )
# 
...
data <- data_all %>% 
  ...
  summarise(number_projects_prim = sum(P_prim_index, na.rm = T),
            perc_projects_prim = 100*number_projects_prim/n()) %>%
