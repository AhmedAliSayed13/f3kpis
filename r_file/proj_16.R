...
# === path to json file 
path <- "RCODE/proj_16.json"

# === read table head
chart <- read_json("proj_16_chart.json")
...
data <- data_all %>% group_by(area_name) %>%
  ...
  mutate_if(is.numeric, round, 2) %>% filter(!is.na(area_name)) %>%
  select(area_name, prob_mashouba, prob_faskh, prob_1warning, prob_2warning, prob_3warning   )
