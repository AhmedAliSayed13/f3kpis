...
# === path to json file 
path <- "RCODE/ind_11.json"

# === read chart json file 
chart <- read_json("ind_11_chart.json")
...
#   group_by(reg_name) %>%
#   summarise(number_projects_faskh = sum(P_faskh_index, na.rm = T) )
# 
...
data <- data_all %>% 
  ...
  summarise(number_projects_faskh = sum(P_faskh_index, na.rm = T),
            perc_projects_faskh = 100*number_projects_faskh/n()) %>%
