...
# === path to json file 
path <- "RCODE/ind_8.json"

# === read chart json file 
chart <- read_json("ind_8_chart.json")
...
#   group_by(reg_name) %>%
#   summarise(number_projects_final = sum(P_final_index, na.rm = T) )
# 
...
data <- data_all %>% 
  ...
  summarise(number_projects_final = sum(P_final_index, na.rm = T),
            percent_projects_final = 100*number_projects_final/n())  %>%
