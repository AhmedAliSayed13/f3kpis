...
# === path to files 
path1 <- "RCODE/report_43.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_43.html"
tble_head <- read.table("report_43_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  group_by(reg_name) %>%
  mutate(sum_duration_reg = sum(P_DURATION, na.rm = T)) %>% ungroup() %>%
  group_by(reg_name,area_name) %>%
  summarise(number_projects = n(),
            sum_duration = sum(P_DURATION, na.rm = T),
            perc_duration = 100*sum(P_DURATION, na.rm = T)/unique(sum_duration_reg)) %>% 
  mutate_if(is.numeric, round,3)  %>% ungroup() %>% select(-reg_name)
