...
# === path to json file 
path <- "RCODE/ind_12.json"

# === read chart json file 
chart <- read_json("ind_12_chart.json")
...
#   group_by(reg_name) %>%
#   summarise(number_projects_mashouba = sum(P_mashouba_index, na.rm = T) )
# 
...
data <- data_all %>% 
  ...
  summarise(number_projects_mashouba = sum(P_mashouba_index, na.rm = T),
            perc_projects_mashouba = 100*number_projects_mashouba/n()) %>%
