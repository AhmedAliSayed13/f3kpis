...
prediction <-  exp(coeff_region[coeff_region$name == input[2],1] + coeff_slope*log(as.numeric(input[1]))) #input[1]

prediction <- round(prediction/1e6,3)  # in million
data <- data.frame(cbind(reg_names[reg_names == input[2]], prediction))
