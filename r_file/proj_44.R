...
# === path to json file 
path <- "RCODE/proj_44.json"

# === read table head
chart <- read_json("proj_43_chart.json")
...
# input <- c(1e6,"الباحة")    # contractor ID
...
model <- lm(log(P_DURATION) ~ -1+factor(reg_name)+log(P_VALUE), data = lm1data) 
