...
# === path to files 
path1 <- "RCODE/report_38.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_38.html"
tble_head <- read.table("report_38_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  group_by(area_name) %>%
  mutate(number_projectsR = n(),
         indx_late = (as.Date("1442-05-15") - (P_ENDD+extra_time))/30 > 18 & 
           (as.Date("1442-05-15") - (P_ENDD+extra_time))/30 < 48 ) %>% 
  filter(is.na(P_PRIMAD) ) %>%
  group_by(area_name) %>%
  summarise(number_projects_reg = unique(number_projectsR),
            number_projects = sum( indx_late , na.rm = T),
            total_late_time = sum( indx_late*(as.Date("1442-05-15") - (P_ENDD+extra_time)), na.rm = T)) %>% 
