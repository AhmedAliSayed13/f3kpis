...
# === path to json file 
path <- "RCODE/proj_46.json"

# === read table head
chart <- read_json("proj_45_chart.json")
...
# input <- c(365,"الباحة")    # contractor ID
...
#================ proj_14 ======================
## استخدام نموذج الانحدار المتعدد للتنبؤ بمدة المشروع
# نهدف هنا الي نمذجة العلاقة بين مدة المشروع (كمتغير تابع) وقيمة المشروع والمنطقة (كمتغيرات مفسرة أو مستقلة) -- هذا بعد محاولات كثيرة لدراسة العلاقات بين المتغيرات والوصول لافضل نموذج.
# شكل الانتشار بين مدة المشروع وقيمة المشروع في الشكل التالي:

lm1data = data_all %>% 
  select(P_VALUE, P_DURATION, reg_name, P_REGION, P_AREA, P_MF,P_CONTID,P_FIND) %>% 
  filter(P_VALUE > 2 & P_VALUE < 6e+07, P_DURATION > 12 & P_DURATION < 2000, P_REGION <17 ) 

model <- lm(log(P_VALUE) ~ -1+factor(reg_name)+log(P_DURATION), data = lm1data) 


reg_names <- c(sort(unique(data_all$reg_name)), 'slope')
coeff <- data.frame(model$coefficients)
coeff$reg_names <- reg_names
rownames(coeff) <- NULL
colnames(coeff) <- c("coef", "name" )
coeff_region <- coeff[-17,]
coeff_slope <- coeff[17,1]



prediction <- c()
for (i in 1:nrow(coeff_region) ){
  prediction[i] <-  exp(coeff_region[i,1] + coeff_slope*log(as.numeric(input[1])))
}
prediction <- round(prediction/1e6,3)  # in million
data <- data.frame(cbind(reg_names[-17], prediction)) 
# data <-  prediction %>% mutate_if(is.numeric, round, 3)
#names(prediction) <- c(chart$head[[1]], chart$head[[2]])
