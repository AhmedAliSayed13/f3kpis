...
# === path to files 
path1 <- "RCODE/report_34.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_34.html"
# tble_head <- read.table("report_34_head.txt", encoding = "UTF-8", sep = "\t")
tble_head <- read.table("report_34_head.txt", sep = "\t")
...
#================ report_34 ======================
data_all$year_STARTD = year(data_all$P_STARTD)
data <- data_all %>% 
  filter((P_ENDD+extra_time) >= input1[1] & (P_ENDD+extra_time) <= input1[2] & !is.na(year_STARTD) ) %>%
  group_by(label = area_name,year_STARTD) %>%
  summarise(number_projects = n()  ) %>%
  spread(year_STARTD, number_projects) 

#colmnames <- names(data)
colnames(data) <- c(tble_head[1], colnames(data[-1]))
data[is.na(data)] = 0
