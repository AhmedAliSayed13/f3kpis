...
# === path to json file 
path <- "RCODE/proj_13.json"

# === read table head
chart <- read_json("proj_13_chart.json")
...
#================ proj_13 ======================
#### التنبؤ بمستوي الانجاز للمشروع كقيمة متوقعة مع فترة ثقة 95% علي مستوي كل ادارة تعليمية
data <- data_all %>%  group_by(area_name) %>% filter(extra_index > 0) %>% 
  summarise(expected = quantile(PROGRESS_A,  probs = c(0.50), na.rm = T) ,
            lower_interval = quantile(PROGRESS_A,  probs = c(0.05), na.rm = T) ,
            uper_interval = quantile(PROGRESS_A,  probs = c(0.95), na.rm = T) ) %>% 
