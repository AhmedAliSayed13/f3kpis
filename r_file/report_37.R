...
# === path to files 
path1 <- "RCODE/report_37.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_37.html"
tble_head <- read.table("report_37_head.txt", sep = "\t")
...
data <- data_all %>%
  ...
  summarise(number_projects_reg = unique(number_projectsR),
            number_projects = sum( extra_index > 0, na.rm = T),
            total_extra_time = sum(extra_time, na.rm = T)) %>% 
