...
# === path to json file 
path <- "RCODE/proj_37.json"

# === read table head
chart <- read_json("proj_37_chart.json")
...
data <- data_all %>% group_by(reg_name, P_CONTID) %>%
  summarise(n_p = n(),
  ...
  mutate_if(is.numeric, round, 3) %>% filter(P_CONTID == input, !is.na(indx)) %>%
  select(-P_CONTID)
