...
# === path to json file 
path <- "RCODE/ind_30.json"

# === read chart json file 
chart <- read_json("ind_30_chart.json")
...
data <- data_all %>% 
  ...
  summarise(number_1warn = sum(P_1warn_index, na.rm = T),
            number_2warn = sum(P_2warn_index, na.rm = T),
            number_3warn = sum(P_3warn_index, na.rm = T)) %>% t()

data <- bind_cols(unlist(chart$colhead), data)
names(data) <- c("label","value")#unlist(chart$head)

data[] <- lapply(data, as.character )

categories <- list(list(category = data[1])) # label
dataset <- list( list(seriesname= chart$head[[2]], data = data[2])   ) # data
