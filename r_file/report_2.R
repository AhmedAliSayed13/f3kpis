...
# === path to files 
path1 <- "RCODE/report_2.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_2.html"
tble_head <- read.table("report_2_head.txt", sep = "\t")
...
data <- data_all %>% 
  ...
  summarise(number_projects = n(),
            number_projectsF =  sum( project_gender == unique(project_gender)[1] ),
            number_projectsM =  sum( project_gender == unique(project_gender)[2] )) %>%
