...
# === path to files 
path1 <- "RCODE/report_1.xlsx"
#path2 <- "RCODE/report_1.docx"
path3 <- "RCODE/report_1.html"
tble_head <- read.table("report_1_head.txt", sep = "\t")
#tble_head <- read.table("report_1_head.txt", encoding = "UTF-8", sep = "\t")
...
# if (is.na(input1[1])) input1 <- c("1426-01-11", "1449-10-11") # year

#================ report_1 ======================
total.number = as.numeric( data_all %>% summarise(n()) )   
data <- data_all %>%
  filter((P_ENDD+extra_time) >= input1[1] & (P_ENDD+extra_time) <= input1[2]) %>%
  group_by(reg_name) %>%
  summarise(number_projects = n(),
            percent_projects = 100*number_projects/total.number ) %>%
...
#colnames(data) <- c("المنطقة",
#                    "عدد المشاريع بالمناطق",
#                    "نسبة المشاريع بالمناطق")

#colnames(data) <- read.table("head.txt", encoding = "UTF-8", sep = "\t")
#write_xlsx(data,path)
#write_tableHTML(tableHTML(data), file = path)
...
#flextable(data)
#save_as_docx("my table" = flextable(data), path = "pptx_file.docx")
#save_as_docx(flextable(data), path = "report1-1.docx")

# 
# colnames(data) <- read.table("head.txt", encoding = "UTF-8")
# data
# 
# 
# colnames <- read.table("head.txt", encoding = "UTF-8", sep = "\t")
# colnames(data) <- colnames[1,]
# dim(colnames)
# colnames[1,]
# class(colnames)
